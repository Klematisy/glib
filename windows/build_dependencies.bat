...
set BROTLI_LIB=-DBROTLIDEC_LIBRARIES=%cd%\extdeps\brotli\lib\brotlidec-static.lib -DBROTLIDEC_INCLUDE_DIRS=%cd%\extdeps\brotli\include
...
cmake --build downloads\brotli\build --parallel --config %BUILD_TYPE%
...
cmake -S downloads\BZip2 -B downloads\BZip2\build ^
    ...
    -DENABLE_DOCS=OFF
cmake --build downloads\BZip2\build --parallel --config %BUILD_TYPE%
...
cmake --build downloads\zlib\build --parallel --config %BUILD_TYPE%
...
cmake --build downloads\libpng\build --parallel --config %BUILD_TYPE%
...
cmake --build downloads\HarfBuzz\build --parallel --config %BUILD_TYPE%
...
cmake --build downloads\freetype\build --parallel --config %BUILD_TYPE%
...
cmake --build downloads\msdf-atlas-gen\build --parallel --config %BUILD_TYPE%
