...
private:
    struct GL {
        ...
        static constexpr int DRAW_BUFFER_TYPE[] = { GL_STATIC_DRAW, GL_DYNAMIC_DRAW };
...
public:
    ...
    void BufferData(BUFFER_TYPE bufferType, uint32_t capacity, void* data, DRAW_BUFFER_TYPE drawType) override;
    void BufferSubData(BUFFER_TYPE bufferType, ptrdiff_t offset, uint32_t size, void* data) override;
    void BindBuffer(BUFFER_TYPE bufferType, uint32_t ID) override;
    void DeleteBuffer(uint32_t count, uint32_t* id) override;

    void CreateVertexArray(uint32_t count, uint32_t* id) override;
    void BindVertexArray(uint32_t id) override;
    void VertexAttribPointer(uint32_t index, uint32_t size, API_TYPE type, bool normalized, uint32_t stride, const void* pointer) override;
    ...
    void Uniform1iv(int location, uint32_t count, int* value) override;
    void UniformMatrix4fv(int location, uint32_t count, API_BOOLEAN transpose, const float* matrix) override;

    void CreateTexture(uint32_t count, uint32_t* textureId) override;
